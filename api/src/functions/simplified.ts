...
type ModelChunkStream = AsyncIterable<ModelChunk>;

app.http('simplified', {
	methods: ['POST'],
	authLevel: 'function',
	handler: async (request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit | HttpResponse> => {
...
function resolveModelResponseFn(streaming: boolean, provider: string) {
...
async function getModelResponse_Mock(userInput: string, _translationInstructions: string): Promise<ModelChunkStream> {
	const mockChunks = [
...
async function getModelResponse(userInput: string, translationInstructions: string): Promise<ModelChunkStream> {
	...
	return stream as unknown as ModelChunkStream;
}

async function getModelResponse_NoStreaming(userInput: string, translationInstructions: string): Promise<ModelChunkStream> {
	const client = new OpenAI({ baseURL: endpoint, apiKey: token });
	...
	return Readable.from([chunk]) as unknown as ModelChunkStream;
}
...
function cleanModelOutput(chunk: ModelChunk) {
