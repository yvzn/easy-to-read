...
interface CarbonFootprintEntity {
	PartitionKey: string;
	RowKey: string;
	InteractionId: string;
	UserInputLength: string;
	UserInputWordCount: string;
	SimplifiedOutputLength: string;
	SimplifiedOutputWordCount: string;
	Duration: string;
}
...
app.http('carbon-footprint', {
	...
	handler: async (request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> => {
		...
		try {
			...
			const rows: CarbonFootprintEntity[] = [];
			rows.push({
				PartitionKey: 'Carbon',
